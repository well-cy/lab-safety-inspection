...
| T09 | 图片检测 | 输入: t05_pcr_scientist.jpg | 返回标注图/人员状态/事件，耗时记录 | persons=1, events=1, 耗时=48.6ms | ✅ |
| T10 | 视频检测 | 输入: demo_lab_video.mp4 (399帧) | 输出标注视频+处理FPS | inferred=200, fps=101.5, events=2, 输出=test_annotated.mp4 | ✅ |
| T11 | 违规截图保存 | 检查 outputs/screenshots 中的视频违规截图 | 违规时生成 jpg 截图文件 | 截图文件数=12 | ✅ |
| T12 | 违规记录写入数据库 | 插入一条测试违规事件后查询 | 能查询到该记录 | 查询到 9 条'未佩戴口罩'记录 | ✅ |
| T13 | 历史记录查询 | 按 severity=严重违规 筛选 | 返回过滤后的记录列表 | 严重违规记录数=7 | ✅ |
| T14 | 数据统计 | 调用 dashboard_stats() | 返回检测数/违规数/违规率/类型分布 | 检测=0, 违规=0, 违规率=0.0% | ❌ |

## 性能指标

- 单张图片处理时间: 48.6 ms
- 视频处理速度（GPU, stride=2, 含绘制/写盘）: 101.5 FPS
- CPU 纯推理速度: 12.5 FPS
