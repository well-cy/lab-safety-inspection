...
4. **新增手套公开数据集（审计通过，待入库）**：① `safety-gloves-xbnf8` v2（Roboflow 官方，3373 张，Gloves/NO-Gloves，CC BY 4.0）：去重审计零重复、与 dataset2 红线零命中，**可全量入库**；② `ppes-kaxsi` v7（11978 张，10 类含 no_glove 负类，CC BY 4.0）：仅 5 张近重复需剔除，可入库 11973。两者均为工厂 CCTV 场景——丁腈手套外观跨域一致，且 **NO-Gloves/no_glove 显式负类正是治理"裸手误检"的关键**；入库时打 `sgv_`/`ppes_` 来源前缀，保留做消融实验的可能。抽检图版：`outputs/safety_gloves_review/`、`outputs/ppes_review/`。入库策略：Gloves/glove→gloves(1)；no_glove/no_* 框不保留、图留作背景负样本（不改 4 类类别表）。
3. **mask 质量良好**：抽检为真实实验室场景、框贴合，无需处理（几何可疑 6 张待顺手过一遍）。
